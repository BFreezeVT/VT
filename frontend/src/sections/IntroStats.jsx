    ...
    <div className="absolute z-30 bottom-full left-0 right-0 mb-2 animate-fade-in" data-testid="source-popup">
      <div className="bg-white border border-[#1a8fd4] p-4 shadow-lg shadow-[#0077B3]/10">
        <div className="flex items-start justify-between gap-3 mb-2">
          <p className="text-[#1a2b42] text-sm font-semibold" style={{ fontFamily: "Outfit" }}>{source.title}</p>
          <button onClick={onClose} className="text-[#4a5e78] hover:text-[#1a2b42] flex-shrink-0"><X className="w-4 h-4" /></button>
        </div>
        <p className="text-[#4a5e78] text-xs leading-relaxed mb-3">{source.excerpt}</p>
        <a href={source.url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-[#0077B3] text-xs font-medium hover:text-[#1a2b42] transition-colors">
  ...
  return (
    <section id="intro-stats" data-testid="intro-stats-section" aria-label="Cybersecurity threat statistics" className="py-24 lg:py-32 bg-white">
      <div className="max-w-7xl mx-auto px-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center mb-20">
          ...
          <div className="relative animate-fade-in-up order-2 lg:order-1">
            <div className="relative overflow-hidden border border-[#FF5722]/20">
              ...
              <div className="absolute bottom-0 left-0 right-0 p-6">
                ...
                <p className="text-[#1a2b42] text-sm font-medium">Attackers are targeting your industry right now. Is your team ready?</p>
              </div>
            </div>
          </div>
          ...
          <div className="order-1 lg:order-2">
            ...
            <div className="border-l-2 border-[#FF5722] pl-5 animate-fade-in-up stagger-3">
              ...
              <p className="text-[#4a5e78] text-sm leading-relaxed">
                Global ransomware attacks rose <span className="text-[#1a2b42] font-semibold">32% in 2025</span>. Average ransom: <span className="text-[#1a2b42] font-semibold">$2.3 million</span>. 90% of organizations are unprepared for AI-augmented threats.
              </p>
              <a href="https://deepstrike.io/blog/ransomware-statistics-2025" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-[#0077B3] text-[10px] mt-2 hover:text-[#1a2b42] transition-colors">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-0 border-t border-[#dfe6ee]/50">
          {stats.map((stat, i) => {
            ...
            return (
              <div
                ...
                className={`relative pt-8 pb-6 ${i < 3 ? "border-r border-[#dfe6ee]/30 hidden lg:block" : ""} ${i < 2 ? "max-lg:border-r max-lg:border-[#dfe6ee]/30" : ""} px-6 first:pl-0 last:pr-0 animate-fade-in-up stagger-${i + 3}`}
                style={{ minWidth: 0 }}
              >
                {activeSource === i && <SourcePopup source={stat.source} onClose={() => setActiveSource(null)} />}
                <Icon className={`w-4 h-4 ${stat.accentColor} mb-3`} />
                <p className="stat-number text-4xl sm:text-5xl text-[#1a2b42]">
                  {stat.value}<span className={stat.accentColor}>{stat.suffix}</span>
                </p>
                <p className="text-xs text-[#4a5e78] mt-2 mb-3 leading-relaxed">{stat.label}</p>
                <button
                  data-testid={`stat-source-${i}`}
                  onClick={() => setActiveSource(activeSource === i ? null : i)}
                  className="inline-flex items-center gap-1 text-[#0077B3] text-[10px] font-medium hover:text-[#1a2b42] transition-colors"
                >
                  View source <ExternalLink className="w-2.5 h-2.5" />
                </button>
                {stat.expandable && (
                  <button data-testid="expand-ranking" onClick={() => setShowRanking(!showRanking)} className="flex items-center gap-1 text-[#0077B3] text-[10px] font-medium hover:text-[#1a2b42] transition-colors mt-1">
        ...
        {showRanking && (
          <div data-testid="top-5-ranking" className="mt-8 pt-8 border-t border-[#dfe6ee]/50 animate-fade-in">
            <p className="text-[#1a2b42] text-sm font-semibold mb-6" style={{ fontFamily: "Outfit" }}>Top 5 Most-Attacked Industries (2025)</p>
            <div className="space-y-4">
              {topAttackedIndustries.map((ind) => (
                <div key={ind.rank} className="flex items-center gap-4">
                  <span className={`stat-number text-lg w-8 ${ind.rank === 1 ? "text-[#FF5722]" : "text-[#4a5e78]"}`}>#{ind.rank}</span>
                  <div className="flex-1">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-[#1a2b42] text-sm font-medium">{ind.name}</span>
                    ...
                    <p className="text-[#4a5e78] text-[10px] mt-1">{ind.note}</p>
                  </div>
                </div>
              ))}
            </div>
            <a href="https://industrialcyber.co/reports/global-ransomware-attacks-rose-32-in-2025-as-manufacturers-emerged-as-top-target/" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-[#0077B3] text-[10px] mt-4 hover:text-[#1a2b42] transition-colors">
