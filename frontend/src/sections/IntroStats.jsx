...
function SourcePopup({ source, onClose }) {
  return (
    <div className="absolute z-30 bottom-full left-0 right-0 mb-2 animate-fade-in" data-testid="source-popup">
      <div className="bg-[#112240] border border-[#1a8fd4] p-4 shadow-lg shadow-[#0077B3]/10">
        <div className="flex items-start justify-between gap-3 mb-2">
          <p className="text-white text-sm font-semibold" style={{ fontFamily: "Outfit" }}>{source.title}</p>
          <button onClick={onClose} className="text-[#d0dce8] hover:text-white flex-shrink-0"><X className="w-4 h-4" /></button>
        </div>
        <p className="text-[#d0dce8] text-xs leading-relaxed mb-3">{source.excerpt}</p>
...
export default function IntroStats() {
  ...
  return (
    <section id="intro-stats" data-testid="intro-stats-section" aria-label="Cybersecurity threat statistics" className="py-24 lg:py-32 bg-[#112240]">
      <div className="max-w-7xl mx-auto px-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center mb-20">
          ...
          <div className="order-1 lg:order-2">
            ...
            <p data-testid="intro-description" className="text-[#d0dce8] text-base leading-relaxed animate-fade-in-up stagger-2 mb-8">
            ...
            <div className="border-l-2 border-[#FF5722] pl-5 animate-fade-in-up stagger-3">
              ...
              <p className="text-[#d0dce8] text-sm leading-relaxed">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-0 border-t border-[#1e6bb8]/50">
          {stats.map((stat, i) => {
            ...
            return (
              <div
                ...
                className={`relative pt-8 pb-6 ${i < 3 ? "border-r border-[#1e6bb8]/30 hidden lg:block" : ""} ${i < 2 ? "max-lg:border-r max-lg:border-[#1e6bb8]/30" : ""} px-6 first:pl-0 last:pr-0 animate-fade-in-up stagger-${i + 3}`}
                style={{ minWidth: 0 }}
              >
                ...
                <p className="text-xs text-[#d0dce8] mt-2 mb-3 leading-relaxed">{stat.label}</p>
                <button
        ...
        {showRanking && (
          <div data-testid="top-5-ranking" className="mt-8 pt-8 border-t border-[#1e6bb8]/50 animate-fade-in">
            ...
            <div className="space-y-4">
              {topAttackedIndustries.map((ind) => (
                <div key={ind.rank} className="flex items-center gap-4">
                  <span className={`stat-number text-lg w-8 ${ind.rank === 1 ? "text-[#FF5722]" : "text-[#d0dce8]"}`}>#{ind.rank}</span>
                  <div className="flex-1">
                    ...
                    <div className="w-full bg-[#112240] h-1"><div className={`h-1 ${ind.rank === 1 ? "bg-[#FF5722]" : "bg-[#0077B3]"}`} style={{ width: ind.pct }} /></div>
                    <p className="text-[#d0dce8] text-[10px] mt-1">{ind.note}</p>
                  </div>
                </div>
              ))}
            </div>
