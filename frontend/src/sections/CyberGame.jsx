...
export default function CyberGame() {
  ...
  return (
    <section id="cyber-game" data-testid="cyber-game-section" className="py-24 lg:py-32 bg-[#001A33]/30">
        ...
        <div className="grid-border-card p-6 sm:p-8 lg:p-10 animate-fade-in-up stagger-3">
          ...
          {gameState === "intro" && (
            <div data-testid="game-intro">
              <div className="text-center mb-8">
                ...
                <p className="text-[#A0B6CD] text-sm max-w-md mx-auto">
                ...
                {Object.entries(DIFFICULTIES).map(([key, cfg]) => {
                  ...
                  return (
                    <button
                      ...
                      className={`p-5 border text-left transition-all hover:border-[#0077B3] hover:bg-[#0077B3]/5 ${difficulty === key ? "border-[#0077B3] bg-[#0077B3]/5" : "border-[#003B71] bg-transparent"}`}
                    >
                      ...
                      <p className="text-[#A0B6CD] text-xs mb-2">
              ...
              {stored.badges.length > 0 && (
                <div className="border-t border-[#003B71] pt-6">
                  <p className="text-xs text-[#A0B6CD] mb-3 uppercase tracking-wider">Your Badges</p>
                  ...
                  <p className="text-[10px] text-[#A0B6CD]/50 mt-2">{stored.gamesPlayed} games played</p>
                </div>
              )}
            </div>
          )}
          ...
          {gameState === "playing" && current && (
            <div data-testid="game-playing">
              ...
              {/* Progress + Timer */}
              <div className="relative w-full bg-[#003B71]/30 h-1.5 mb-6">
              ...
              {config.timer > 0 && !answered && (
                <div className="flex items-center gap-2 mb-4">
                  <Clock className={`w-3 h-3 ${timeLeft <= 5 ? "text-[#FF5722] animate-pulse" : "text-[#A0B6CD]"}`} />
                  <div className="flex-1 bg-[#003B71]/30 h-1">
                    <div
                      className={`h-1 transition-all duration-1000 ${timeLeft <= 5 ? "bg-[#FF5722]" : "bg-[#0077B3]"}`}
                      style={{ width: `${(timeLeft / config.timer) * 100}%` }}
                    />
                  </div>
                  <span className={`text-xs font-mono ${timeLeft <= 5 ? "text-[#FF5722]" : "text-[#A0B6CD]"}`}>{timeLeft}s</span>
                </div>
              )}
              ...
              <div className="bg-[#020812] border border-[#003B71] p-5 sm:p-6 mb-6">
                ...
                <div className="border-t border-[#003B71] pt-4 mt-3">
                  <p data-testid="email-preview" className="text-[#A0B6CD] text-sm leading-relaxed">{current.preview}</p>
                </div>
              </div>
              ...
              {!answered ? (
                <div className="grid grid-cols-2 gap-4">
                  <Button data-testid="game-btn-legitimate" onClick={() => handleAnswer(false)} className="bg-transparent border border-[#003B71] hover:border-[#0077B3] hover:bg-[#0077B3]/10 text-white rounded-sm h-12 font-semibold">
                    <ShieldCheck className="w-4 h-4 mr-2" /> Legitimate
                  </Button>
                  <Button data-testid="game-btn-phishing" onClick={() => handleAnswer(true)} className="bg-transparent border border-[#003B71] hover:border-[#FF5722] hover:bg-[#FF5722]/10 text-white rounded-sm h-12 font-semibold">
          ...
          {gameState === "result" && (
            <div data-testid="game-results" className="text-center py-4">
              ...
              <p className="text-xs text-[#A0B6CD] mb-1">{config.label} Mode</p>
              ...
              <div className="grid grid-cols-3 gap-3 mb-6">
                <div className="bg-[#001A33] border border-[#003B71] p-3">
                  <p className="stat-number text-xl text-white">{Math.round((score / shuffledEmails.length) * 100)}%</p>
                  <p className="text-[10px] text-[#A0B6CD]">Accuracy</p>
                </div>
                <div className="bg-[#001A33] border border-[#003B71] p-3">
                  ...
                  <p className="text-[10px] text-[#A0B6CD]">Best Streak</p>
                </div>
                <div className="bg-[#001A33] border border-[#003B71] p-3">
                  <p className="stat-number text-xl text-[#0077B3]">{highScore}%</p>
                  <p className="text-[10px] text-[#A0B6CD]">Personal Best</p>
                </div>
              </div>
              ...
              <p className="text-[#A0B6CD] text-sm mb-6">
                {score === shuffledEmails.length
              ...
              <div className="flex flex-col sm:flex-row gap-3 justify-center mb-4">
                <Button data-testid="game-restart-button" onClick={() => setGameState("intro")} className="bg-transparent border border-[#003B71] hover:border-[#0077B3] text-white rounded-sm font-semibold px-5 h-10">
                  <RotateCcw className="w-4 h-4 mr-2" /> Play Again
                </Button>
                <Button data-testid="game-share-button" onClick={shareResults} className="bg-transparent border border-[#003B71] hover:border-[#0077B3] text-white rounded-sm font-semibold px-5 h-10">
                  <Share2 className="w-4 h-4 mr-2" /> Share Score
                </Button>
                <Button data-testid="game-cta-button" onClick={() => document.getElementById("audit")?.scrollIntoView({ behavior: "smooth" })} className="bg-white text-[#003B71] hover:bg-white/90 rounded-sm font-semibold px-5 h-10">
                  Get Your Free Audit
                </Button>
              </div>
            </div>
