    ...
    <section id="ai-service" data-testid="ai-service-section" aria-label="AI as a Service offerings" className="py-24 lg:py-32 bg-[#e9eff6] border-t border-[#c8d6e5] relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="max-w-3xl mb-16">
            <div className="relative flex items-center justify-center mb-10"><div className="absolute inset-0 flex items-center"><div className="w-full border-t-2 border-[#003B71]/20"></div></div><span className="relative bg-[#e9eff6] px-6 text-[#003B71] text-sm font-bold uppercase tracking-[0.2em]">AI as a Service</span></div>
          ...
          <div className="flex items-center gap-6">
            <div>
              <p className="stat-number text-3xl sm:text-4xl text-[#1a2b42]">90<span className="text-[#FF5722]">%</span></p>
              <p className="text-xs text-[#4a5e78] mt-1">unprepared for AI-augmented threats</p>
            </div>
            <div className="w-px h-12 bg-[#1e6bb8]/50" />
            <div>
              <p className="stat-number text-3xl sm:text-4xl text-[#1a2b42]">68<span className="text-[#0077B3]">%</span></p>
              <p className="text-xs text-[#4a5e78] mt-1">of employees use unauthorized AI tools</p>
            </div>
          </div>
          <a href="https://newsroom.accenture.com/news/2025/only-one-in-10-organizations-globally-are-ready-to-protect-against-ai-augmented-cyber-threats" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-[#0077B3] text-[10px] mt-3 hover:text-[#1a2b42] transition-colors">
            Sources: Accenture 2025, Second Talent 2025 <ExternalLink className="w-2.5 h-2.5" />
          </a>
        </div>

        {/* Services — alternating open layout */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-12 gap-y-10 border-t border-[#d0dbe8]/50 pt-12">
          {aiServices.map((svc, i) => (
            <div key={svc.title} data-testid={`ai-card-${i}`} className={`animate-fade-in-up stagger-${(i % 6) + 1}`}>
              ...
              <h3 className="text-[#1a2b42] font-semibold text-base mb-2" style={{ fontFamily: "Outfit, sans-serif" }}>{svc.title}</h3>
