...
export default function BlogPost() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#020812] flex items-center justify-center">
        <p className="text-[#A0B6CD]">Loading...</p>
      </div>
    );
  }
  ...
  const renderContent = (text) => {
    return text.split("\n\n").map((block, i) => {
      ...
      if (block.startsWith("- ")) {
        ...
        return (
          <ul key={i} className="space-y-2 my-4 ml-4">
            {items.map((item, j) => (
              <li key={j} className="text-[#A0B6CD] text-base leading-relaxed flex items-start gap-2">
      ...
      if (block.match(/^\d+\./)) {
        ...
        return (
          <ol key={i} className="space-y-2 my-4 ml-4 list-decimal list-inside">
            {items.map((item, j) => (
              <li key={j} className="text-[#A0B6CD] text-base leading-relaxed" dangerouslySetInnerHTML={{ __html: formatInline(item.replace(/^\d+\.\s*/, "")) }} />
            ))}
          </ol>
        );
      }
      ...
      return (
        <p key={i} className="text-[#A0B6CD] text-base leading-relaxed my-4" dangerouslySetInnerHTML={{ __html: formatInline(block) }} />
      );
    });
  };
  ...
  return (
    <div className="min-h-screen bg-[#020812]" data-testid={`blog-post-${post.slug}`}>
      <script
      ...
      <nav className="bg-[#020812]/95 backdrop-blur-md border-b border-[#003B71] sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-6">
            <Link to="/resources" className="text-[#A0B6CD] hover:text-white text-sm flex items-center gap-1">
        ...
        <article className="py-20 lg:py-28" aria-label={post.title}>
          <div className="max-w-3xl mx-auto px-6">
            ...
            <div className="flex items-center gap-4 mb-6">
              ...
              <span className="flex items-center gap-1 text-[#A0B6CD]/60 text-xs">
                <Clock className="w-3 h-3" /> {post.read_time}
              </span>
              <span className="text-[#A0B6CD]/60 text-xs">{post.published_date}</span>
            </div>
      ...
      <footer className="bg-[#020812] border-t border-[#003B71] py-8">
        <div className="max-w-7xl mx-auto px-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-[#A0B6CD]/60 text-xs">&copy; {new Date().getFullYear()} Veracity Technologies.</p>
