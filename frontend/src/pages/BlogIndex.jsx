...
export default function BlogIndex() {
  ...
  return (
    <div className="min-h-screen bg-[#0a1628]" data-testid="blog-index">
      <nav className="bg-[#0a1628]/95 backdrop-blur-md border-b border-[#0d4a8a] sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-6">
            <Link to="/" className="text-[#b0c4d8] hover:text-white text-sm flex items-center gap-1">
      ...
      <main role="main">
        <section className="py-24 lg:py-32">
          <div className="max-w-7xl mx-auto px-6">
            ...
            <p className="text-[#b0c4d8] text-base max-w-2xl mb-16">
              Practical guides, threat analysis, and compliance insights from the Veracity Technologies team.
            </p>

            {loading ? (
              <div className="text-[#b0c4d8] text-sm">Loading articles...</div>
            ) : (
                ...
                {posts.map((post, i) => (
                  ...
                  >
                    <div className="flex items-center gap-3 mb-4">
                      <span className={`text-[10px] uppercase tracking-wider border px-2 py-0.5 ${categoryColors[post.category] || "text-[#0077B3] border-[#0d4a8a]"}`}>
                        {post.category}
                      </span>
                    ...
                    <p className="text-[#b0c4d8] text-sm leading-relaxed mb-4 flex-1">
                      {post.excerpt}
                    </p>
                    <div className="flex items-center justify-between mt-auto pt-4 border-t border-[#0d4a8a]">
                      <span className="text-xs text-[#b0c4d8]/60">{post.published_date}</span>
